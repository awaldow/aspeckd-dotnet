...
> **Looking for just the abstractions?** See [`Aspeckd.Core`](https://www.nuget.org/packages/Aspeckd.Core) for the companion package that carries no ASP.NET Core dependency.
...
- [`Aspeckd.Core` package](https://www.nuget.org/packages/Aspeckd.Core) — stable abstractions with no ASP.NET Core dependency
